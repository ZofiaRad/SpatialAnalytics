...
- wall and object coordinates in JSON/GeoJSON format,
- manually annotated room polygons,
...
The repository contains three main R Markdown analysis scripts and a set of Python conversion utilities.
...
Contains Python utilities used to convert between JSON, GeoJSON, and trajectory CSV formats. These scripts support the spatial preprocessing pipeline and make it possible to align the trajectory data with the museum geometry.
...
- separating stationary viewing time from walking or circulation time,
- including artwork size, visual salience, period, artist, or popularity as additional predictors,
...
- making the preprocessing pipeline more reproducible with clearer path conventions and dependency documentation,
- adding a requirements file or setup instructions for the R and Python environments,
- documenting the expected order in which scripts should be run.
